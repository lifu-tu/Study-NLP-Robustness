...
class BERTClassifier(Block):
    ...
    def __init__(self, bert, num_classes=2, dropout=0.0, prefix=None, params=None):
        ...
        with self.name_scope():
            ...
            #self.classifier.add(nn.Dense(units=1024))
...
class FixBERTClassifier(Block):
    ...
    """
    def __init__(self, bert, num_classes=2, dropout=0.0, prefix=None, params=None):
        super(FixBERTClassifier, self).__init__(prefix=prefix, params=params)
        self.bert = bert
        with self.name_scope():
            self.classifier = nn.HybridSequential(prefix=prefix)
            if dropout:
    ...
    def forward(self, inputs, token_types, valid_length=None): # pylint: disable=arguments-differ
        ...
        outputs : NDArray
            Shape (batch_size, num_classes)
        """
        output, _ = self.bert(inputs, token_types, valid_length)
